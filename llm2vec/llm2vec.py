...
class LLM2Vec(nn.Module):
    ...
    def forward(self, sentence_feature: Dict[str, Tensor],is_q=False):
        embed_mask = None
        ...
        return self.get_pooling(sentence_feature, reps.last_hidden_state,is_q=is_q)


    def get_pooling(self, features, last_hidden_states, is_q=False):
        ...
        elif self.pooling_mode == "M_token":
            if not is_q:
                return last_hidden_states[:, -8:]
            else:
                return last_hidden_states[:, -1]
        else:
        ...
        if torch.cuda.device_count() <= 1:
            ...
            for start_index in trange(
                ...
                embeddings = self._encode(
                    sentences_batch, device=device, convert_to_numpy=convert_to_numpy, is_q=is_q
                )
            ...
            with cuda_compatible_multiprocess.Pool(num_proc) as p:
                ...
                for batch in sentences_batches:
                    results.append(
                        p.apply_async(
                            self._encode,
                            args=(batch, None, convert_to_numpy, True, is_q),
                            callback=update,
                        )
                    )
    ...
    def _encode(
        ...
        with torch.no_grad():
            embeddings = self.forward(features, is_q=is_q)
